...
# For wind bias
from pyrorl.envs.environment.environment_constant import set_fire_mask, linear_wind_transform
#from src.environment.barriers import add_barrier
...
# Goal: Adjust the sample_fire propagation function to 
# set the proablity of barrier cells going on fire to zero
...
class FireWorld:
    ...
    def sample_fire_propogation(self, barriers):
        ...
        # If we have barriers, set their fire_index to zero (zero probability of burning)
        if len(barriers) != 0:
            for (row, col) in barriers:
                self.state_space[FIRE_INDEX][row][col] = 0
    ...
    def advance_to_next_timestep(self, barriers):
        """
        Take three steps:
